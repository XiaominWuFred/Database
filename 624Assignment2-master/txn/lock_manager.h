...
class LockManagerB : public LockManager {
 public:
  ...
  void grant(Txn *txn);
    ...
    void reduceWaitList(Txn *txn);

    void eraseRTxnIfContain(Txn *txn);
    vector<LockManagerB::LockRequest>* findvec(deque<LockManager::LockRequest>* reqQue,deque<LockManager::LockRequest>::iterator itr);
};
